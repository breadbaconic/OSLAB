...
sys_env_set_status(envid_t envid, int status)
{
    ...
    struct Env *envptr;
    int result;
    if((result = envid2env(envid,&envptr,1))<0){
        return result;
    }
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if((r=envid2env(envid,&dstenv,0))<0){
        cprintf("envid does not exist\n");
        return -E_BAD_ENV;
    }
    if((!dstenv->env_ipc_recving)||dstenv->env_ipc_from>0){
       // cprintf("envid not blocked\n");
        return -E_IPC_NOT_RECV;
    }
    if(srcva<(void *)UTOP){
        if(ROUNDDOWN(srcva,PGSIZE) != srcva){
            cprintf("page not aligned\n");
            return -E_INVAL;
        }
        if((perm&(PTE_U|PTE_P))!=(PTE_U|PTE_P)){
            cprintf("perm inappropriate\n");
            return -E_INVAL;
        }
        if(perm&(~PTE_SYSCALL)){
            cprintf("perm inappropriate\n");
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
   ...
   // cprintf("syscallno = %d\n",syscallno);
    ...
    case SYS_cgetc:
       result =  sys_cgetc();break;
    case SYS_getenvid:
       result = sys_getenvid();break;
    case SYS_env_destroy:
        result = sys_env_destroy((envid_t)a1);break;
    case SYS_page_alloc:
        result = sys_page_alloc(a1,(void *)a2,a3);break;
    case SYS_page_map:
        result = sys_page_map(a1,(void *)a2,a3,(void *)a4,a5);break;
    case SYS_page_unmap:
        result = sys_page_unmap(a1,(void *)a2);break;
    case SYS_exofork:
        result = sys_exofork();break;
    case SYS_env_set_status:
        result = sys_env_set_status(a1,a2);break;
    case SYS_env_set_pgfault_upcall:
        result = sys_env_set_pgfault_upcall(a1,(void*)a2);break;
    case SYS_yield:
        sys_yield();break;
    case SYS_ipc_try_send:
        result = sys_ipc_try_send(a1,a2,(void *)a3,a4);break;
    case SYS_ipc_recv:
        result = sys_ipc_recv((void *)a1);break;
    default:
        result = -E_INVAL;
    }
    return result;
